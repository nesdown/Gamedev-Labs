namespace Assets.Lab4.FallowDeer.Behaviours
{
    ...
    public class Wander : DesiredVelocityProvider
    {
        ...
        public override Vector3 GetDesiredVelocity()
        {
            ...
            var futurePos = FallowDeer.transform.position + FallowDeer.Velocity.normalized * circleDistance;
            var vector = new Vector3(Mathf.Cos(angle * Mathf.Deg2Rad), Mathf.Sin(angle * Mathf.Deg2Rad), 0) * circleRadius;

            return (futurePos + vector - transform.position).normalized * FallowDeer.VelocityLimit;
        }
    }
}