 ...
 */
const Snake = function(game, spriteKey, x, y, ang, s, name, id) {
    this.game = game;
    this.id = id;
    ...
    const tintKey = (+spriteKey[0] === 1) ? 1 : (+spriteKey[0] === 2) ? 2 : (+spriteKey[0] === 3) ? 3 : 0;
    this.shadow = new Shadow(this.game, this.sections, this.scale, tintKey);
    ...
    this.edgeOffset = 4;
    this.edge = this.game.add.sprite(x, y - this.edgeOffset, 'circle'+this.spriteKey[0]);
    this.edge.name = "edge";
    this.edge.alpha = 0;
    this.game.physics.p2.enable(this.edge, this.debug);
    this.edge.body.setCircle(this.edgeOffset);
    this.edge.body.collideWorldBounds = true;
...
Snake.prototype = {
     ...
     */
    initSections: function(num, ang) {
	const dx = Util.cos(ang) * this.preferredDistance;
	const dy = Util.sin(ang) * this.preferredDistance;
        //create a certain number of sections behind the head
        //only use this once
        for (var i = 1 ; i <= num ; i++) {
            var x = this.head.body.x + i * dx;
            var y = this.head.body.y + i * dy;
...
//            var y = this.head.body.y + i * this.preferredDistance;
    ...
    addSectionAtPosition: function(x, y) {
        ...
        const i = this.spriteKey.shift();
        var sec = this.game.add.sprite(x, y, 'circle'+i);
        this.spriteKey.push(i);
        ...
        sec.body.collideWorldBounds = true;
     ...
     */
    update: function() {
        if (Math.abs(this.head.body.x) * 2 > 4000 || Math.abs(this.head.body.y) * 2 > 4000) return this.destroy();
        ...
        if (this.growing === false && this.snakeLength - 2 <= (this.price - this.basePrice) / this.growth) {
            this.growing = true;
    ...
    destroy: function() {
        ...
        //call this snake's destrusocketioction callbacks
